...
export const getAllUsers = createAsyncThunk<IGetAllUserResponse>(
	"users/getallusers", async () => {
		const api = new HttpClientApplication();
		const response = await api.Get<IGetAllUserResponse>(
			RouteHttps.identity.users.getallusers
		);
		return response;
	});

export const createUser = createAsyncThunk<ICreateUserResponse, ICreateUserRequest>(
	"users/createuser", async (createUserRequest: ICreateUserRequest) => {
		const api = new HttpClientApplication();
		const response = await api.Post<ICreateUserResponse>(
			RouteHttps.identity.users.createuser, createUserRequest
		);
		return response;
	});

export const updateUser = createAsyncThunk<IUpdateUserResponse, IUpdateUserRequest>(
	"users/updateuser", async (updateUserRequest: IUpdateUserRequest) => {
		const api = new HttpClientApplication();
		const response = await api.Patch<IUpdateUserResponse>(
			RouteHttps.identity.users.updateuser, updateUserRequest
		);
		return response;
	});

export const deleteUser = createAsyncThunk<IDeleteUserResponse, IDeleteUserRequest>(
	"users/delete", async (deleteUserRequest: IDeleteUserRequest) => {
		const api = new HttpClientApplication();
		const response = await api.Delete<IDeleteUserResponse>(
			RouteHttps.identity.users.deleteuser + deleteUserRequest.id
		);
		return response;
	});

export const getUserById = createAsyncThunk<IGetUserByIdResponse, IGetUserByIdRequest>(
	"users/getuserbyid", async (getUserByIdRequest: IGetUserByIdRequest) => {
		const api = new HttpClientApplication();
		const response = await api.Get<IGetUserByIdResponse>(
			RouteHttps.identity.users.getUserbyid + getUserByIdRequest.id
		);
		return response;
	});